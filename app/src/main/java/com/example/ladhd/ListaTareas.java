    ...
    public ListaTareas(Context context) {
        ...
        textToSpeech = new TextToSpeech(context, status -> {
            if (status == TextToSpeech.SUCCESS) {
                ...
                if (result == TextToSpeech.LANG_MISSING_DATA || result == TextToSpeech.LANG_NOT_SUPPORTED) {Toast.makeText(context, "El idioma no es compatible para TTS", Toast.LENGTH_SHORT).show();}
                else {isTextToSpeechReady = true;}
            } else {Toast.makeText(context, "Error al inicializar TTS", Toast.LENGTH_SHORT).show();}
        });
    }
    private static final DiffUtil.ItemCallback<Tarea> CALLBACK = new DiffUtil.ItemCallback<Tarea>() {
        @Override public boolean areItemsTheSame(@NonNull Tarea oldItem, @NonNull Tarea newItem) {return oldItem.getId() == newItem.getId();}
